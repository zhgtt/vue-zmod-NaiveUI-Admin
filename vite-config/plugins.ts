/**
 * @description: vite 相关的所有插件都在这里定义 & 注册
 */
...
// * vur-router v5版本以上整合了 unplugin-vue-router 插件，需要注释和替换掉之前的相关代码
...
import vueDevTools from 'vite-plugin-vue-devtools' // 本地调试工具

// 引入自定义的插件
import { RouterWatcherPlugin } from './custom-plugins'

/**
 * @description: 封装 vite 插件的引入和注册 👇
 * @param viteEnv - vite 环境变量
 * @param isBuild - TODO 是否为打包状态，默认为 false（目前还用不到）
 */
export function setupVitePlugins(viteEnv: Env.ImportMeta) {
  ...
  const vitePlugins: (PluginOption | PluginOption[])[] = [
    ...
    RouterWatcherPlugin(), // 自定义插件 - 文件变化时，自动更新路由
